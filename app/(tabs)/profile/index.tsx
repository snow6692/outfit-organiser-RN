...
import { useGetUserOutfits, useCreateOutfit, useDeleteOutfit } from '~/hooks/useOutfitQueries';
...
const WardrobeScreen: React.FC = () => {
  ...
  const generateAndSaveOutfits = async () => {
    ...
    try {
      ...
      const errorMessage = error.message.includes('You can only use your own images')
        ? 'Some selected items are not available. Please choose different items.'
  ...
  const renderOutfitItem = ({
    ...
    item: { id: string; images: { id: string; url: string; Category: { name: string } }[] };
  }) => (
